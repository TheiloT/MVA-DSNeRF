...
def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if savedir is not None:
            ...
            depth = to8b(depth / np.percentile(depth, 95))
...
def train():
    ...
    if args.dataset_type == 'colmap_llff':
        ...
        # print("train_images:")   # (N_train, H, W, 3) (e.g. N=2 for 2 views)
        # print(train_imgs.shape)
        # print(train_imgs)
        # print(20*"=")
        # print("test_images:")   # (N_test, H, W, 3) (e.g. N=3)
        # print(test_imgs.shape)
        # print(test_imgs)
        # print(20*"=")
        # print("train_poses:")   # (N_train, 3, 5) (this is the extrinsic and (condensed) intrinsic matrix concatenated. Thus there are 5 columns because the last column contains the camera width, height and focal length)
        # print(train_poses.shape)
        # print(train_poses)
        # print(20*"=")
        # print("test_poses:")   # (N_test, 3, 5)
        # print(test_poses.shape)
        # print(test_poses)
        # print(20*"=")
        # print("render_poses:")   # (N_render, 3, 5) (e.g. N=120)
        # print(render_poses.shape)
        # print(render_poses)
        # print(20*"=")
        # print("depth_gts:")   # (N_train) where each element is a dictionary with keys: 'depth', 'coord' and 'error' (these are the depth for the sparse colmap keypoints with errors)
        # print(depth_gts.shape)
        # print(depth_gts)
        # print(20*"=")
        # print("bds:")   # (2) (scalars for near and far bounds)
        # print(bds.shape)
        # print(bds)
        # print(20*"=")
    ...
    for i in trange(start, N_iters):
        ...
        if i%args.i_print==0:
            if args.depth_loss and args.sigma_loss:
                tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}\n RGB Loss: {img_loss.item()}  Depth Loss: {depth_loss.item()} Sigma Loss: {sigma_loss.item()}")
            elif args.depth_loss:
                tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}\n RGB Loss: {img_loss.item()}  Depth Loss: {depth_loss.item()}")
            elif args.sigma_loss:
                tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}\n RGB Loss: {img_loss.item()}  Sigma Loss: {sigma_loss.item()}")
            else:
                tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR: {psnr.item()}\n")
